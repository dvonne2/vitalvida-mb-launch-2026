...
only pay Affiliate Commission Events that carry a submitted Purchase Invoice, so
the payable being settled provably exists in ERPNext AP. Paying un-accrued
commission is refused: that is exactly how money moves without a ledger entry.
    ...
    return frappe.db.sql("""
        SELECT e.name, e.vv_order, e.commission_amount
         ...
         WHERE e.media_buyer = %(buyer)s
           ...
           AND e.purchase_invoice IS NOT NULL AND e.purchase_invoice != ''
...
def record_payout(payout_batch, external_reference, approved_by=None):
    ...
    if not lines:
        frappe.throw(
            ...
            "and accrued (Purchase Invoice posted) before it can be paid.")
