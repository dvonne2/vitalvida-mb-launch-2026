...
Purchase Invoice, no GL Entry. It also stores payout state in two places at once.
...
def guard_order_payout_status(doc, method=None):
    ...
    accrued = frappe.db.get_value("Affiliate Commission Event",
                                  {"vv_order": doc.name}, ["name", "purchase_invoice"],
                                  as_dict=True)
    if not accrued or not accrued.purchase_invoice:
        frappe.throw(
            f"Order {doc.name} cannot be marked affiliate-Paid: no accrued "
            "Affiliate Commission Event with a Purchase Invoice exists. "
            "Commission must be earned and accrued before it is paid.",
            frappe.ValidationError)
    if frappe.db.get_value("Purchase Invoice", accrued.purchase_invoice, "docstatus") != 1:
        frappe.throw(
            f"Order {doc.name} cannot be marked affiliate-Paid: Purchase Invoice "
            f"{accrued.purchase_invoice} is not submitted, so the payable does not "
            "exist in the ledger.",
            frappe.ValidationError)
