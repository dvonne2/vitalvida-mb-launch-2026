...
class SettlementApprovalEvent(Document):
    ...
    def before_save(self):
        if not self.is_new():
            ...
            if old and any(getattr(old,f,None)!=getattr(self,f,None) for f in immutable if self.meta.has_field(f)):
                frappe.throw("Settlement Approval Event records are immutable.", frappe.PermissionError)
    def on_trash(self):
        frappe.throw("Settlement Approval Event records cannot be deleted.", frappe.PermissionError)
