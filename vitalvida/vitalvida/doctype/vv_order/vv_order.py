...
class VVOrder(Document):
	...
	def _compute_delivery_fee(self):
		"""Fetch delivery fee from Delivery Fee Config (Single DocType)."""
		if not self.delivery_type:
			return
		try:
			config = frappe.get_single("Delivery Fee Config")
			if self.delivery_type == "Same Day":
				self.delivery_fee = config.same_day_fee or 0
			else:
				self.delivery_fee = config.standard_fee or 0
		except Exception:
			pass
